...
umap<string, u_p<BaseMap::Terrain>> BaseMap::terrains;


void BaseMap::AddTerrain(u_p<Terrain> terrain)
{
	terrains.emplace(terrain->name, move(terrain));
}


BaseMap::BaseMap(string name, int width, int height)
		: name(move(name))
		, width(width)
		, length(length)
{
...
BaseMap::BaseMap(const string& path, string name)
		: name(move(name))
{
...
BaseMap::BaseMap(const BaseMap& other)
		: name(other.name)
		, width(other.width)
		, length(other.length)
		, cells(other.cells)
...
void BaseMap::ChangeTerrain(Cell& cell, const string& terrainName)
{
	cell.terrain = terrains[terrainName].get();
}

void BaseMap::ChangeObject(Cell& cell, Object* object)
{
	cell.object.reset(object);
}

void BaseMap::SaveToFile(const string& path) const
{
	ofstream fout(path);
	if (!fout)
...
void BaseMap::LoadFromFile(const string& path)
{
	ifstream fin(path);
	if (!fin)
